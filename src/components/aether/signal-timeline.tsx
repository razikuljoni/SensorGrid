...
function actionIcon(action: string): { icon: typeof Activity; color: string } {
  if (action.startsWith('device.command')) return { icon: Zap, color: 'text-accent-foreground bg-accent/30' }
  if (action.startsWith('device.create') || action.startsWith('device.update')) return { icon: Cpu, color: 'text-info bg-info/10' }
  ...
  if (action.startsWith('device.online') || action.startsWith('device.offline')) {
    return action.includes('online') ? { icon: Power, color: 'text-success bg-success/10' } : { icon: Power, color: 'text-text-muted bg-muted' }
  }
  if (action.startsWith('alert')) return { icon: AlertTriangle, color: 'text-warning bg-warning/10' }
  if (action.startsWith('automation')) return { icon: Activity, color: 'text-primary bg-primary/10' }
...
export function SignalTimeline({ events, maxItems = 50, className, emptyMessage = 'No activity yet' }: SignalTimelineProps) {
      ...
      <div className="relative pl-2">
        ...
        <ul className="space-y-3">
          {items.map((evt, i) => {
            ...
            return (
              <li key={evt.id} className={cn('relative flex items-start gap-3 aether-blip', i === 0 && 'animate-in')} style={{ animationDelay: i === 0 ? '0s' : undefined }}>
                <span className={cn('z-10 flex size-5 shrink-0 items-center justify-center rounded-full ring-2 ring-background', color)}>
                ...
                <div className="flex-1 min-w-0 pt-0.5">
                  <div className="flex items-baseline justify-between gap-2">
                    <p className="text-sm font-medium truncate">
                      ...
                      {evt.targetName && <span className="text-text-muted font-normal"> · {evt.targetName}</span>}
                    </p>
                    <time className="text-[10px] text-text-muted whitespace-nowrap shrink-0">{timeAgo(evt.createdAt)}</time>
                  </div>
