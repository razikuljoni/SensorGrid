...
export function TelemetryTile({
  ...
  return (
    <div
      className={cn(
        'aether-tile relative overflow-hidden rounded-xl border border-border bg-surface transition-colors hover:border-border-subtle',
        compact ? 'p-2' : 'p-3',
        className
      )}
    >
      <div className="flex items-center justify-between gap-1.5">
        <div className="flex items-center gap-1 min-w-0 flex-1">
          ...
          <span
            className={cn(
              'font-medium text-text-muted leading-tight',
      ...
      <div className="mt-1 flex items-baseline gap-0.5 flex-wrap">
        <span
          className={cn(
            'font-semibold tabular-nums tracking-tight transition-colors',
            compact ? 'text-base' : 'text-lg',
            flicker && 'aether-flicker'
          )}
        >
      ...
      {timestamp && !compact && (
        <div className={cn('mt-0.5 text-[9px]', qualityColor)}>
