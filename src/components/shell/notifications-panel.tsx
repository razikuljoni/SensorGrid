...
const CATEGORY_META: Record<NotificationCategory, { icon: typeof Bell; color: string; label: string }> = {
      ...
      <SheetContent side="right" className="w-full sm:w-96 p-0 flex flex-col">
        <SheetHeader className="px-4 py-3 border-b border-border flex-row items-center justify-between space-y-0">
          <div className="flex items-center gap-2">
            ...
            {unread > 0 && <Badge className="bg-danger text-danger-foreground h-5 px-1.5 text-[10px]">{unread} unread</Badge>}
          </div>
          <Button variant="ghost" size="sm" className="text-xs h-7 gap-1" onClick={markAllRead} disabled={unread === 0}>
            <CheckCheck className="size-3" /> Mark all read
          </Button>
        </SheetHeader>
        <SheetDescription className="sr-only">In-app notifications across categories</SheetDescription>
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          {!n.read && <span className="mt-1 size-1.5 shrink-0 rounded-full bg-danger" />}
                        </div>
