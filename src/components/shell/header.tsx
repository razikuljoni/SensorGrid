...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-14 sm:h-16 items-center gap-2 sm:gap-3 border-b border-border bg-background/80 px-3 sm:px-4 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      ...
      {/* Search */}
      <Button variant="outline" className="hidden lg:flex w-56 xl:w-64 justify-start gap-2 text-text-muted shrink-0" onClick={() => setCommandPaletteOpen(true)}>
      ...
      {/* Theme toggle */}
      <Button variant="ghost" size="icon" className="shrink-0" onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')} aria-label="Toggle theme">
        <Sun className="size-4 rotate-0 scale-100 transition-all dark:-rotate-90 dark:scale-0" />
        <Moon className="absolute size-4 rotate-90 scale-0 transition-all dark:rotate-0 dark:scale-100" />
      </Button>

      {/* Notifications */}
      <Button variant="ghost" size="icon" className="relative shrink-0" onClick={() => setNotificationsOpen(true)} aria-label="Open notifications">
