...
function CustomTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="rounded-lg border border-border bg-popover px-3 py-2 text-xs shadow-md">
      <div className="font-medium text-text-secondary mb-1">{new Date(label).toLocaleString(undefined, { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</div>
      {payload.map((p: any, i: number) => (
        <div key={i} className="flex items-center justify-between gap-3">
          ...
          <span className="font-medium tabular-nums">{typeof p.value === 'number' ? p.value.toFixed(1) : p.value} {p.payload?.unit ?? ''}</span>
        </div>
      ))}
    </div>
  )
    ...
    <div className="w-full">
      ...
      <ResponsiveContainer width="100%" height={height}>
        <Chart data={data} margin={{ top: 8, right: 16, bottom: 0, left: -16 }}>
          ...
          <XAxis dataKey="timestamp" tickFormatter={formatXAxis} stroke="var(--text-muted)" fontSize={10} tickLine={false} axisLine={false} minTickGap={40} />
          ...
          {series.map((s, i) => {
            ...
            if (variant === 'area') {
              return (
                <Area key={s.sensorKey} type="monotone" dataKey={s.sensorKey} name={sensorMeta(s.sensorKey).label} stroke={color} fill={color} fillOpacity={0.15} strokeWidth={1.5} isAnimationActive={false} dot={false} connectNulls />
              )
            }
            return <Line key={s.sensorKey} type="monotone" dataKey={s.sensorKey} name={sensorMeta(s.sensorKey).label} stroke={color} strokeWidth={1.5} isAnimationActive={false} dot={false} connectNulls />
          })}
        </Chart>
      </ResponsiveContainer>
    </div>
