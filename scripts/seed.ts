...
const DEVICES: DeviceSeed[] = [
  {
    ...
    credential: {
      username: 'dev-living-esp32',
      password: 'nx_live_9f3a',
      clientId: 'dev-living-esp32',
    },
  },
  {
    id: 'dev-kitchen-rpi',
    ...
    credential: {
      username: 'dev-kitchen-rpi',
      password: 'nx_kitchen_7c2e',
      clientId: 'dev-kitchen-rpi',
    },
  },
  {
    ...
    credential: {
      username: 'dev-greenhouse-esp',
      password: 'nx_green_b8d1',
      clientId: 'dev-greenhouse-esp',
    },
  },
  {
    id: 'dev-bedroom-esp',
    ...
    credential: {
      username: 'dev-bedroom-esp',
      password: 'nx_bed_2a5f',
      clientId: 'dev-bedroom-esp',
    },
  },
  {
    ...
    credential: {
      username: 'dev-office-arduino',
      password: 'nx_office_4d9c',
      clientId: 'dev-office-arduino',
    },
  },
  {
    ...
    sensors: [{ key: 'motion', label: 'Motion', unit: '', dataType: 'boolean', min: 0, max: 1 }],
    twinDesired: { door: 'closed' },
    twinReported: { door: 'open', lastPosition: 'open' },
    credential: {
      username: 'dev-garage-generic',
      password: 'nx_garage_e7f3',
      clientId: 'dev-garage-generic',
    },
  },
  {
    ...
    credential: {
      username: 'dev-server-gateway',
      password: 'nx_server_a1b2',
      clientId: 'dev-server-gateway',
    },
  },
  {
    ...
    credential: {
      username: 'dev-living-presence',
      password: 'nx_presence_8e4d',
      clientId: 'dev-living-presence',
    },
  },
];
...
async function main() {
  ...
  for (const seed of DEVICES) {
    ...
    await db.deviceCredential.upsert({
      ...
      update: {
        username: seed.credential.username,
        password: seed.credential.password,
        clientId: seed.credential.clientId,
      },
      create: {
        deviceId: seed.id,
        username: seed.credential.username,
    ...
    await db.deviceTwin.upsert({
      where: { deviceId: seed.id },
      update: {
        desired: JSON.stringify(seed.twinDesired),
        reported: JSON.stringify(seed.twinReported),
      },
      create: {
        deviceId: seed.id,
        desired: JSON.stringify(seed.twinDesired),
  ...
  for (const device of DEVICES) {
    ...
    for (const sensor of device.sensors) {
      ...
      for (let i = 144; i >= 0; i--) {
        ...
        const dailySwing =
          sensor.key === 'temperature'
            ? Math.sin((hourOfDay / 24) * Math.PI * 2 - Math.PI / 2) * 4
            : 0;
        ...
        await db.telemetry.create({
          data: {
            deviceId: device.id,
            sensorId: `${device.id}__${sensor.key}`,
  ...
  const alertRules = [
    ...
    },
    {
      ...
      name: 'Critical Server Temp',
      deviceId: 'dev-server-gateway',
      sensorKey: 'temperature',
      condition: 'GT',
      threshold: 60,
      severity: 'CRITICAL',
    ...
    {
      id: 'rule-battery-low',
      name: 'Battery Below 20%',
      deviceId: 'dev-greenhouse-esp',
      sensorKey: null,
    ...
    },
    {
      ...
      name: 'Garage Device Offline',
      deviceId: 'dev-garage-generic',
      sensorKey: null,
      condition: 'OFFLINE_FOR',
      threshold: 5,
      severity: 'CRITICAL',
    },
    {
  ...
  await db.alertEvent.create({
    data: {
      ...
      deviceId: 'dev-server-gateway',
      ruleName: 'Critical Server Temp',
      severity: 'CRITICAL',
      status: 'TRIGGERED',
      message: 'Server room temperature exceeded 60C threshold.',
      context: JSON.stringify({ value: 67.4, threshold: 60 }),
    },
  });
  await db.alertEvent.create({
    data: {
      ...
      deviceId: 'dev-garage-generic',
      ruleName: 'Garage Device Offline',
      severity: 'CRITICAL',
      status: 'ACKNOWLEDGED',
  ...
  const automationNodes = [
    {
      ...
      data: {
        ...
        config: { deviceId: 'dev-living-esp32', sensorKey: 'temperature' },
      },
    },
    {
      id: 'n2',
      ...
      position: { x: 280, y: 0 },
      data: {
        label: 'Temperature > 30C',
        kind: 'gt',
        config: { sensorKey: 'temperature', threshold: 30 },
      },
    },
    {
      id: 'n3',
      ...
      data: {
        label: 'Turn Fan ON',
        kind: 'send_command',
        config: { deviceId: 'dev-living-esp32', payload: { fan: true } },
      },
    },
    {
      id: 'n4',
      ...
      position: { x: 560, y: 80 },
      data: {
        label: 'Notify: High Temp',
        kind: 'notify',
        config: {
          category: 'ALERT',
          title: 'High Temperature Alert',
          message: 'Living room fan activated.',
        },
      },
    },
  ];
  ...
  await db.automation.upsert({
    ...
    update: {
      ...
      triggerConfig: JSON.stringify({ deviceId: 'dev-living-esp32', sensorKey: 'temperature' }),
      nodes: JSON.stringify(automationNodes),
      edges: JSON.stringify(automationEdges),
    },
    create: {
      id: 'auto-cooling',
      organizationId: org.id,
  ...
  const co2Nodes = [
    {
      ...
      data: {
        label: 'Telemetry: CO2',
        kind: 'telemetry_received',
        config: { deviceId: 'dev-kitchen-rpi', sensorKey: 'co2' },
      },
    },
    {
      id: 'n2',
      type: 'condition',
      position: { x: 280, y: 0 },
      data: { label: 'CO2 > 1000 ppm', kind: 'gt', config: { sensorKey: 'co2', threshold: 1000 } },
    },
    {
      id: 'n3',
      type: 'action',
      ...
      data: {
        label: 'Vent ON',
        kind: 'send_command',
        config: { deviceId: 'dev-kitchen-rpi', payload: { vent: true } },
      },
    },
  ];
  await db.automation.upsert({
    ...
    update: {
      ...
      triggerType: 'TELEMETRY',
      triggerConfig: JSON.stringify({ deviceId: 'dev-kitchen-rpi', sensorKey: 'co2' }),
      nodes: JSON.stringify(co2Nodes),
  ...
  const offlineNodes = [
    {
      ...
      data: {
        ...
        kind: 'device_offline',
        config: { deviceId: 'dev-garage-generic' },
      },
    },
    {
      id: 'n2',
      ...
      position: { x: 320, y: 0 },
      data: {
        label: 'Notify: Device Offline',
        kind: 'notify',
        config: {
          category: 'DEVICE',
          title: 'Device Offline',
          message: 'Garage Door Controller is offline.',
        },
      },
    },
  ];
  await db.automation.upsert({
    ...
    update: {
      ...
      triggerConfig: JSON.stringify({ deviceId: 'dev-garage-generic' }),
      nodes: JSON.stringify(offlineNodes),
      edges: JSON.stringify([{ id: 'e1', source: 'n1', target: 'n2' }]),
    },
    create: {
      id: 'auto-offline-alert',
      organizationId: org.id,
      name: 'Device Offline Alert',
      enabled: false,
      description: 'Sends a notification when the garage controller goes offline.',
  ...
  const notifs = [
    {
      category: 'ALERT',
      title: 'Critical Server Temperature',
      message: 'Server room gateway reached 67.4C (threshold 60C).',
    },
    {
      category: 'DEVICE',
      title: 'Garage Device Offline',
      message: 'Garage Door Controller has been offline for 2 hours.',
    },
    {
      category: 'AUTOMATION',
      title: 'Smart Cooling Executed',
      message: 'Living room fan activated due to temperature threshold.',
    },
    {
      category: 'SYSTEM',
      title: 'Welcome to SensorGrid',
      message: 'Your demo workspace is ready. Connect a device to begin.',
    },
  ];
  ...
  const audits = [
    {
      ...
      targetType: 'DEVICE',
      targetId: 'dev-server-gateway',
      targetName: 'Server Room Gateway',
    ...
    },
    {
      ...
      targetId: 'auto-cooling',
      targetName: 'Smart Cooling',
      actorName: 'SensorGrid Operator',
      metadata: {},
    },
    {
      ...
      targetType: 'ALERT',
      targetId: null,
      targetName: 'Garage Device Offline',
      actorName: 'SensorGrid Operator',
      metadata: {},
    },
    {
      ...
      targetType: 'COMMAND',
      targetId: null,
      targetName: 'Living Room ESP32',
      actorName: 'SensorGrid Operator',
      metadata: { payload: { fan: true } },
    },
  ];
  ...
  await db.command.create({
    data: {
      deviceId: 'dev-living-esp32',
      userId: user.id,
      senderName: 'SensorGrid Operator',
      ...
      acknowledgedAt: new Date(Date.now() - 29 * 60_000),
      completedAt: new Date(Date.now() - 29 * 60_000),
    },
  });
  await db.command.create({
    data: {
      deviceId: 'dev-office-arduino',
      userId: user.id,
      senderName: 'SensorGrid Operator',
      payload: JSON.stringify({ outlet2: true }),
      topic: 'sensorgrid/sensorgrid-hq/dev-office-arduino/command',
      status: 'FAILED',
      error: 'Device rejected payload: outlet2 not configurable while outlet3 is active.',
      attempts: 1,
